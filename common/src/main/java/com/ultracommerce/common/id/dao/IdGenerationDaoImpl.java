...
public class IdGenerationDaoImpl implements IdGenerationDao {
    ...
    @Override
    @Transactional(value = "ucTransactionManager", propagation = Propagation.REQUIRES_NEW)
    public IdGeneration findNextId(String idType, Long batchSize) throws OptimisticLockException, Exception {
        ...
        Query query = em.createNamedQuery("UC_FIND_NEXT_ID");
        query.setParameter("idType", idType);
        try {
            IdGeneration idGeneration =  (IdGeneration) query.getSingleResult();
            response =  (IdGeneration) entityConfiguration.createEntityInstance("com.ultracommerce.profile.core.domain.IdGeneration");
