...
class ListGridModule extends Module {
    ...
    def scrollToIndex(int number) {
        if (id == null || id.isEmpty()) {
            js.exec 'UCAdmin.listGrid.paginate.scrollToIndex($(\'.listgrid-body-wrapper tbody\'), ' + number + ');'
        } else {
            js.exec 'UCAdmin.listGrid.paginate.scrollToIndex($(\'.' + id + ' .listgrid-body-wrapper tbody\'), ' + number + ');'
        }
    }
